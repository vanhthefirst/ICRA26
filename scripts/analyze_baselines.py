...
def probe_split(rows_expl, rows_amb, rows_probe):
    """The `there` -> `that` probe, paired trial by trial.

    The pooled deictic split (deictic_split) cannot establish causation: the
    three templates containing `there` also share a target deictic and a verb,
    so anything that distinguishes them is confounded with the word. This
    compares the ambiguous caption against the same caption with only that word
    changed, on the same scenes at the same rollout indices, which is the
    comparison that does separate them.

    `n_that_only` / `n_there_only` are the discordant pairs; `p_value` is the
    exact two-sided McNemar test on them. A null result here means the pooled
    split was a correlation, and the ambiguous arm should NOT be discounted for
    a lexical artefact."""
    from math import comb
    tp, ta = _by_trial(rows_probe), _by_trial(rows_amb)
    te = _by_trial(rows_expl)
    shared = sorted(set(tp) & set(ta))
    if not shared:
        return None
    hit = lambda d, k: d[k]["success_sustained"] == "True"
    n = len(shared)
    a = sum(1 for k in shared if hit(tp, k) and not hit(ta, k))
    b = sum(1 for k in shared if not hit(tp, k) and hit(ta, k))
    m = a + b
    p = (sum(comb(m, i) for i in range(min(a, b) + 1)) / 2 ** m * 2) if m else 1.0
    return dict(
        n_trials=n, n_scenes=len({(s, d) for s, d, _ in shared}),
        ambiguous=100.0 * sum(1 for k in shared if hit(ta, k)) / n,
        probe=100.0 * sum(1 for k in shared if hit(tp, k)) / n,
        explicit=(100.0 * sum(1 for k in shared if k in te and hit(te, k)) / n
                  if all(k in te for k in shared) else None),
        n_that_only=a, n_there_only=b, p_value=min(p, 1.0),
        seed_matched=100.0 * sum(1 for k in shared
                                 if tp[k]["init_state_hash"] == ta[k]["init_state_hash"]) / n,
    )
...
def write_tables(path, arms, paired_stats=None, deictic=None, buckets=None,
    ...
    if buckets:
        lines.append(f"\\newcommand{{\\tabBucket}}{{%\n{bucket_rows(buckets)}}}")
    if probe:
        for macro, val in [("Amb", f(probe["ambiguous"])),
                           ("That", f(probe["probe"])),
                           ("Delta", f(probe["probe"] - probe["ambiguous"])),
                           ("Expl", f(probe["explicit"])),
                           ("Scenes", i(probe["n_scenes"])),
                           ("Trials", i(probe["n_trials"])),
                           ("ThatOnly", i(probe["n_that_only"])),
                           ("ThereOnly", i(probe["n_there_only"])),
                           ("SeedMatch", f(probe["seed_matched"])),
                           ("P", f(probe["p_value"], 2))]:
            lines.append(f"\\newcommand{{\\numProbe{macro}}}{{{val}}}")
...
def report_probe(p):
    print("\n===== probe: `there` -> `that`, same scenes and seeds =====")
    print(f"  {i(p['n_scenes'])} scenes, {i(p['n_trials'])} paired trials, "
          f"seed pairing {f(p['seed_matched'])}%")
    print(f"  ambiguous  (`there`)  {f(p['ambiguous']):>6}")
    d = p["probe"] - p["ambiguous"]
    print(f"  probe      (`that`)   {f(p['probe']):>6}   delta {d:+.1f}")
    print(f"  explicit   (ceiling)  {f(p['explicit']):>6}")
    print(f"  discordant: that-only {i(p['n_that_only'])}, "
          f"there-only {i(p['n_there_only'])}; McNemar exact p = {f(p['p_value'], 3)}")
    if p["p_value"] > 0.05:
        print("  -> NOT significant. The pooled deictic split was a correlation:")
        print("     the word is not what those templates lose on.")
...
def main():
    ...
    ap.add_argument("--probe", default=None, metavar="RUNDIR",
                    help="the `there`->`that` diagnostic arm "
                         "(--prompt-type ambiguous_that). Paired trial by trial "
                         "against the ambiguous arm on the scenes it covers.")
    args = ap.parse_args()
    ...
    paired_stats, deictic, buckets, probe = None, None, None, None
    if "explicit" in arms and "ambiguous" in arms:
        ...
        if args.probe:
            pdir = (args.probe if os.path.isabs(args.probe)
                    else os.path.join(RUN_ROOT, args.probe))
            prows = load_rows(pdir, "ambiguous_that")
            if not prows:
                print(f"[warn] no rows for probe at {pdir}")
            else:
                probe = probe_split(raw["explicit"], raw["ambiguous"], prows)
                report_probe(probe)
