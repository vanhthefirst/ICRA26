...
value (`scripts/capture_scene_init_states.py`, `TOL_PX = 1.0`, actual
...
(`rollout_sketch.py:open_scene_env`).
...
- **Rung 2, rendered depth** (`depth`; `depth_z_at_pixel` in
  `rollout_sketch.py`). Constructs the env with `camera_depths=[True]`,
...
`capture_scene_init_states.py` and `rollout_sketch.py` import it.

## Issue 5 — sustained success, and the permissive `In`

- **Sustained window.** `SUCCESS_WINDOW = 5` consecutive steps
  (`rollout_sketch.py`). `results.csv` records both `success_final` (last
...
`rollout_sketch.py`) — the minimum a policy perceiving the raw RGB frame
...
- `env.close()` + `gc.collect()` between scenes (`rollout_sketch.py`'s
...
python scripts/capture_scene_init_states.py --smoke        # 3-scene vertical slice
python scripts/capture_scene_init_states.py

# 2. vertical slice: one scene per suite, NoOp then oracle
python scripts/rollout_sketch.py --smoke --policy noop
python scripts/rollout_sketch.py --smoke --policy oracle
...
python scripts/rollout_sketch.py --conditions human_consensus --policy oracle \
